...
{
    static class PackageBinaryZipping
    {
        /// <summary>
        ...
        static bool Zip(string directoryPath, string[] filenames, string zipFilePath)
        {
            var zipper = Get7zPath();

            zipFilePath = Path.GetFullPath(zipFilePath);
            File.Delete(zipFilePath);
            var inputArguments = String.Format("a -tzip -mx3 \"{0}\" {1}", zipFilePath, String.Join(" ", filenames.Select(f => "\"" + f + "\"").ToArray()));
        ...
        static string Get7zPath()
        {
#if (UNITY_EDITOR_OSX || UNITY_EDITOR_LINUX)
            string execFilename = "7za";
#else
            var execFilename = "7z.exe";
#endif
            var zipper = EditorApplication.applicationContentsPath + "/Tools/" + execFilename;
        ...
        {
            var zipper = Get7zPath();
            var inputArguments = string.Format("x -y -o\"{0}\" \"{1}\"", destPath, zipFilePath);
